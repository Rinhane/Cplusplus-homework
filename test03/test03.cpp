...
const int HURT = 10;
const int HEAL = 10;    // 血包回复减少

const int NEED_STAR = 3;
const int ATTACK_DURATION = 10;
const int WIN_SCORE = 5000; // 胜利分数
...
void showRules(int w, int h)
{
    ...
    while (true)
    {
        ...
        const TCHAR* line5 = _T("空格发射子弹，按R返回主界面");
        const TCHAR* line6 = _T("达到5000分即可获胜");  // 新增：获胜条件提示
        ...
        outtextxy((w - textwidth(line6)) / 2, h / 2 + 100, line6);  // 新增这行
...
void showWin(int w, int h)
{
    ...
    while (true)
    {
        ...
        // ---------------- 1. 图片居中 ----------------
        int img_w = 400;  // 你可以按需要调整大小，比如改成 500
        int img_h = 320;  // 高度跟着比例改，比如 400
        img_win.Resize(img_w, img_h);

        // 水平、垂直都居中
        int img_x = (w - img_w) / 2;
        int img_y = (h - img_h) / 2 - 50;  // 往上挪一点，避免文字被挤到屏幕外
        ...
        int text_y = img_y + img_h + 30; // 图片下方 30 像素
        outtextxy(text_x, text_y, winText);

        // 提示文字（屏幕底部居中）
        settextstyle(24, 0, _T("微软雅黑"));
        const TCHAR* tip = _T("点击任意位置退出");
...
void showMainUI(int w, int h)
{
    ...
    while (true)
    {
        ...
        if (now - lastKeyTime > 200)
        {
            ...
            {
                diff = EASY; lastKeyTime = now; return;
            }
            if (GetAsyncKeyState('2') & 0x8000)
            {
                diff = NORMAL; lastKeyTime = now; return;
            }
            if (GetAsyncKeyState('3') & 0x8000)
            {
                diff = HARD; lastKeyTime = now; return;
            }
...
void showRules(int w, int h)
{
    ...
    while (true)
    {
        ...
        const TCHAR* line4 = _T("攻击期间角色变为哈气形态");
        const TCHAR* line5 = _T("空格发射子弹，按R返回主界面");
        const TCHAR* line6 = _T("简单10000分/普通6000分/困难4000分获胜");
    ...
    while (true)
    {
        ...
        if (MouseHit())
        {
            ...
            if (m.uMsg == WM_LBUTTONDOWN)
            {
                ...
                {
                    diff = EASY;
                    WIN_SCORE = 10000; //简单10000
                    return;
                }
                if (mx >= x && mx <= x + bw && my >= y2 && my <= y2 + bh)
                {
                    diff = NORMAL;
                    WIN_SCORE = 6000;  //中等6000
                    return;
                }
                if (mx >= x && mx <= x + bw && my >= y3 && my <= y3 + bh)
                {
                    diff = HARD;
                    WIN_SCORE = 4000;  //困难4000
                    return;
                }
                if (mx >= x && mx <= x + bw && my >= y4 && my <= y4 + bh)
                {
        ...
        if (now - lastKeyTime > 200)
        {
            ...
            if (GetAsyncKeyState('2') & 0x8000)
            {
                diff = NORMAL; WIN_SCORE = 6000; lastKeyTime = now; return;
            }
            if (GetAsyncKeyState('3') & 0x8000)
            {
                diff = HARD; WIN_SCORE = 4000; lastKeyTime = now; return;
            }
            if (GetAsyncKeyState('4') & 0x8000)
            {
                showRules(w, h); lastKeyTime = now;
